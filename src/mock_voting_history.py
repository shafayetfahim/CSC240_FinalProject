...
def generate_proxy_votes(reps_csv="../data/new_data/member_ideology_cleaned.csv",
                         bills_csv="../data/new_data/bills_categorized.csv",
                         output_csv="../data/new_data/voting_history.csv"):
    """
    Generates a proxy voting history to test the Apriori and BOAT algorithms.
    Maps real icpsrs to real Bill Categories with randomized voting behavior.
    """
    try:
        logging.info("Loading real icpsrs and Bill Categories...")
        reps = pd.read_csv(reps_csv)
        bills = pd.read_csv(bills_csv)

        # We need a subset of Reps so the Apriori matrix isn't too sparse for testing
        test_reps = reps['icpsr'].dropna().sample(n=100, random_state=42).tolist()
        categories = bills['Category_Label'].unique().tolist()

        history = []
        logging.info("Simulating voting receipts for Market Basket Analysis...")

        for icpsr in test_reps:
            for cat in categories:
                # Simulate a 60% chance they voted 'Yes' on a bill in this category
                voted_yes = np.random.choice([0, 1], p=[0.4, 0.6])
                history.append({'icpsr': icpsr, 'Category_Label': cat, 'Voted_Yes': voted_yes})
